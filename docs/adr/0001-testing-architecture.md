...
> Historical note: this ADR was written for the platform template this repo was
> cloned from; its call/SMS examples refer to that template's app. The decision
> (the `verify` field, the layer model, and signed native artifacts) is what
> `packages/spec-test` implements and still governs testing here. For
> DriveBuddy, the `native` layer covers background GPS capture and push
> delivery rather than call/SMS interception.
