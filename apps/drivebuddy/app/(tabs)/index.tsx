...
export default function HomeScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      api.unreadCount().then(({ count }) => setUnread(count)).catch(() => undefined);
      api.insights().then(setInsights).catch(() => undefined);
    }, []),
  );

  const tiles: { title: string; desc: string; route: string; icon: IoniconName; tint: string; badge?: number }[] = [
    { title: "Recommendations", desc: "Insights from your drives", route: "/recommendations", icon: "bulb-outline", tint: accent.carpark },
    { title: "Notifications", desc: "Alerts and trip summaries", route: "/notifications", icon: "notifications-outline", tint: accent.traffic, badge: unread },
    { title: "My Vehicles", desc: "Manage your vehicles", route: "/vehicles", icon: "car-outline", tint: accent.fuel },
    { title: "Profile", desc: "Your personal details", route: "/profile", icon: "person-outline", tint: accent.weather },
    { title: "Settings", desc: "App and account", route: "/settings", icon: "settings-outline", tint: colors.textMuted },
  ];

  return (
    <SafeAreaView style={styles.container} edges={["bottom"]}>
      <ScrollView contentContainerStyle={styles.inner}>
        <Hero title={`Hi ${user?.fullName?.split(" ")[0] ?? "there"}`} subtitle="Here's your week on the road">
